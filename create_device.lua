...
function createDevice:activate(app)
	for _, v in pairs(createDevice.hotkeys) do
		v:enable()
	end
...
function createDevice:_rebuildPresets()
	...
	if createDevice.presetCommand then
		local fileList = hs.execute(createDevice.presetCommand)
...
function createDevice:_rebuildDevices()
	...
	if createDevice.app:getMenuItems() == nil then
		return devices
				...
				for k = 1, #submenu do
					if not (submenu[k]["AXTitle"] == "") then -- table contains divider bars, which have a blank title
						...
						table.insert(devices, {
							["text"] = title,
							["subText"] = string.format("%s - %s", menus[i]["AXTitle"], subtitle),
							["menuSelector"] = {
								"Create",
								menus[i]["AXTitle"],
								subtitle,
